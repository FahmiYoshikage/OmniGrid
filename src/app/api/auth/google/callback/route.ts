...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (requestState.linkUserId) {
            ...
            return NextResponse.redirect(
                buildPublicUrl('/settings?auth=google-linked', request.url)
            );
        }
        ...
        const userId = resolveUserForSignIn(
            ...
            {
                allowTrustedEmailMatch: Boolean(
        ...
        return NextResponse.redirect(
            buildPublicUrl('/auth/success?provider=google', request.url)
        );
    } catch (error) {
        ...
        if (requestState.linkUserId) {
            return NextResponse.redirect(
                buildPublicUrl(
                    '/settings?authError=google-link-failed',
                    request.url
                )
            );
        }
