...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (requestState.linkUserId) {
      ...
      return NextResponse.redirect(new URL("/settings?auth=github-linked", request.url));
    }

    const userId = upsertGitHubUser(ghUser);
    await createSession(userId);

    return NextResponse.redirect(new URL("/auth/success?provider=github", request.url));
  } catch (error) {
    console.error("[auth] GitHub OAuth callback error:", error);
    if (requestState.linkUserId) {
      return NextResponse.redirect(new URL("/settings?authError=github-link-failed", request.url));
    }
