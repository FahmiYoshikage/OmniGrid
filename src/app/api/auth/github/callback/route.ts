...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (requestState.linkUserId) {
      ...
      return NextResponse.redirect(buildPublicUrl("/settings?auth=github-linked", request.url));
    }

    const userId = upsertGitHubUser(ghUser);
    await createSession(userId);

    return NextResponse.redirect(buildPublicUrl("/auth/success?provider=github", request.url));
  } catch (error) {
    console.error("[auth] GitHub OAuth callback error:", error);
    if (requestState.linkUserId) {
      return NextResponse.redirect(buildPublicUrl("/settings?authError=github-link-failed", request.url));
    }
