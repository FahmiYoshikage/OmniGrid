...
export async function GET(request: NextRequest) {
  ...
  if (!tokenParam) {
    return NextResponse.redirect(buildPublicUrl("/login?error=invalid_email_link", request.url));
  }
  ...
  try {
    if (payload.linkUserId) {
      ...
      return NextResponse.redirect(buildPublicUrl("/settings?auth=email-linked", request.url));
    }
    ...
    return NextResponse.redirect(buildPublicUrl("/auth/success?provider=email", request.url));
  } catch (error) {
    console.error("[auth] Email verification error:", error);
    if (payload.linkUserId) {
      return NextResponse.redirect(buildPublicUrl("/settings?authError=email-link-failed", request.url));
    }
