...
export function ContainersClient({ snapshot }: ContainersClientProps) {
  ...
  async function fetchContainers(showSpinner = true) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/uptime/discover", { cache: "no-store" });
      const data = (await res.json().catch(() => ({}))) as { containers?: ContainerEntry[]; error?: string };
      if (!res.ok) {
        throw new Error(data.error || "Failed to scan docker containers");
      }
      setContainers(data.containers ?? []);
      setLastRefreshed(new Date());
    } catch (err) {
      const message = err instanceof Error ? err.message : "Failed to scan docker containers";
      setError(message);
      setContainers([]);
    } finally {
      setLoading(false);
  ...
  const deviceSummary = useMemo(() => {
    ...
    const online = snapshot?.devices.filter((device) => device.online).length ?? 0;
  ...
  const filtered = useMemo(() => {
    ...
    const base = containers.filter((container) => hostFilter === "all" || container.source === hostFilter);
    const searched = normalizedQuery
      ? base.filter((container) =>
          [container.name, container.image, container.source].some((value) =>
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <RefreshCw className={cn("h-4 w-4", scanning && "animate-spin")} /> Scan Docker
            </button>
            <Link
              href="/uptime"
      ...
      <div className="grid grid-cols-1 gap-4 p-8 xl:grid-cols-[1.2fr_1fr]">
          ...
          <CardHeader className="flex-row items-start justify-between gap-4">
            <div>
              <CardTitle className="flex items-center gap-2 text-base">
                <ShieldCheck className="h-4 w-4 text-cyan-200" /> Tailscale fleet status
              </CardTitle>
              <p className="mt-1 text-sm text-muted-foreground">
                Live snapshot of every device connected to your tailnet.
              </p>
            </div>
            {snapshot ? (
              <Badge variant="secondary">{deviceSummary.online}/{deviceSummary.total} online</Badge>
            ) : (
          ...
          <CardContent>
            {!snapshot ? (
              <div className="flex flex-col items-center gap-4 rounded-2xl border border-white/10 bg-black/40 p-6 text-center">
                ...
                <div>
                  <p className="text-sm font-medium text-white">Tailscale integration not configured</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Add your Tailscale API key and tailnet in Settings to see fleet status here.
                  </p>
                </div>
                <Link
                  href="/settings"
                ...
                >
                  <Settings className="h-3 w-3" /> Configure Tailscale
                </Link>
              </div>
            ) : (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {snapshot.devices.map((device) => (
                  <div key={device.id} className="rounded-2xl border border-white/10 bg-black/30 p-3">
                    <div className="flex items-center gap-2">
                      <span
                        className={cn(
                          "h-2.5 w-2.5 rounded-full",
                          device.online ? "bg-emerald-500 shadow-emerald-500/50" : "bg-zinc-500",
                        )}
                      />
                      <span className="text-sm font-medium text-white truncate">{device.shortName}</span>
                      {device.online ? (
                        <Badge variant="secondary" className="ml-auto text-[10px]">Online</Badge>
                      ) : (
                        <Badge variant="outline" className="ml-auto text-[10px]">Offline</Badge>
                      )}
                    </div>
                    <div className="mt-2 grid gap-1 text-xs text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>IP</span>
                        <span className="font-mono text-white/80">{device.tailscaleIp || "-"}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span>OS</span>
                        <span className="text-white/70">{device.os || "Unknown"}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span>Last seen</span>
        ...
        <Card className="border-white/10 bg-white/[0.04] shadow-2xl shadow-black/10">
          <CardHeader className="flex-row items-start justify-between gap-4">
            <div>
              <CardTitle className="flex items-center gap-2 text-base">
                <Boxes className="h-4 w-4 text-emerald-200" /> Container overview
              </CardTitle>
              <p className="mt-1 text-sm text-muted-foreground">
                Docker containers discovered across every node connected to omnigrid-net.
              </p>
            </div>
            <Badge variant="secondary">{containerSummary.running}/{containerSummary.total} running</Badge>
          </CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-3">
            <SummaryTile label="Total containers" value={String(containerSummary.total)} icon={<Boxes className="h-4 w-4" />} />
            <SummaryTile label="Running" value={String(containerSummary.running)} icon={<Wifi className="h-4 w-4" />} />
            <SummaryTile label="Stopped" value={String(containerSummary.exited)} icon={<Server className="h-4 w-4" />} />
          </CardContent>
        </Card>
      </div>
      ...
      <div className="px-8 pb-8">
        <Card className="border-white/10 bg-white/[0.04] shadow-2xl shadow-black/10">
          <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <CardTitle className="flex items-center gap-2 text-base">
                <Boxes className="h-4 w-4 text-cyan-200" /> Containers
              </CardTitle>
              <p className="mt-1 text-sm text-muted-foreground">
                Filter by host, search by name or image, and create monitors for anything in or outside the tailnet.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-muted-foreground">
                ...
                >
                  {hosts.map((host) => (
                    <option key={host} value={host} className="bg-slate-900 text-white">
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="flex flex-col items-center gap-4 rounded-2xl border border-white/10 bg-black/30 p-8 text-center">
                ...
                <div>
                  <p className="text-sm font-medium text-white">No containers found</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Ensure your services run on the omnigrid-net Docker network or refresh the scan.
                  </p>
                </div>
                <button
                  onClick={() => void fetchContainers(true)}
                ...
                <table className="w-full border-separate border-spacing-y-2 text-left text-xs">
                  ...
                  <tbody>
                    {filtered.map((container) => {
                      ...
                      return (
                        <tr key={container.id} className="rounded-2xl bg-black/30">
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2">
                              <span
                                className={cn(
                                  "h-2.5 w-2.5 rounded-full",
                                  running ? "bg-emerald-500 shadow-emerald-500/50" : "bg-zinc-600",
                                )}
                              />
                              <div>
                                <div className="font-medium text-white">{container.name}</div>
                                <div className="text-[10px] text-muted-foreground">{container.id.slice(0, 12)}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-3 py-3 text-white/80">{container.source}</td>
                          <td className="px-3 py-3 text-white/70">{container.image}</td>
                          <td className="px-3 py-3">
                            <span
                              className={cn(
                                "inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px]",
                                running ? "bg-emerald-500/15 text-emerald-300" : "bg-zinc-500/15 text-zinc-300",
                              )}
                            >
                              {container.status || container.state}
                            </span>
                          </td>
                          <td className="px-3 py-3 text-white/70">{container.ports || "-"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
